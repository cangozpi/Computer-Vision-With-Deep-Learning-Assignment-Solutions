...
def affine_forward(x, w, b):
    ...
    M = b.shape[0]
    D = np.prod(x.shape[1:])
    out = np.dot(x.reshape(N,D), w) + b.reshape(1, M)
...
def affine_backward(dout, cache):
    ...
    x = x.reshape(N, D) # Flatten feature vectors of x
    
    # partial derivatives (local gradients)
    dx = w.copy()
    dw = (x.T).copy()
    ...
    #reshape dx to correspond to unflattened x shape
...
def relu_backward(dout, cache):
    ...
    # dRelu(x)/dx (calculate the local gradient)
    dx = np.array(x > 0)

    # dLoss/dRelu(x) * dRelu(x)/dx (apply the chain rule)
    dx = dout * dx
...
def leaky_relu_backward(dout, cache):
    ...
    mask = np.where(x > (alpha*x), 1, alpha)

    dx = np.array(dout) * mask
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        mask = (np.random.randn(*x.shape) < p) / p # inverted dropout, scaling done during training time
        out =  x * mask
...
def dropout_backward(dout, cache):
    ...
    if mode == 'train':
        ...
        #dx = 0 if output dropped, dx=1 if output passed 
        dx = dout * mask
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    steps_to_right = int((((2 * conv_param["pad"]) + W - WW) / conv_param["stride"]) + 1)
    steps_to_down = int((((2 * conv_param["pad"]) + H - HH) / conv_param["stride"]) + 1)
    
    out = np.zeros((N, F, steps_to_down, steps_to_right))

    # iterate through every training instance
    for n in range(N):
      # iterate right in each row
      for r in range(0, steps_to_right):
        # iterate down in each column
        for d in range(0, steps_to_down):
          # iterate through every Filter
          for f in range(F):
            current_x = x_padded[n,:, (d * conv_param["stride"]):((d * conv_param["stride"])+HH), (r * conv_param["stride"]):((r * conv_param["stride"])+WW)]
            current_filter = w[f]
            current_bias = b[f]
            #convolve with current filter
            conv_out = np.sum(current_x * current_filter)
            # add the bias
            conv_out += current_bias
            out[n, f, d, r] = conv_out
...
def max_pool_forward_naive(x, pool_param):
    ...
    steps_to_right = int(((W - WW) / stride) + 1)
    steps_to_down = int(((H - HH) / stride) + 1)
    
    out = np.zeros((N, C, steps_to_down, steps_to_right))
    ...
    for n in range(N):
      ...
      for r in range(0, steps_to_right):
        ...
        for d in range(0, steps_to_down):
          ...
          for c in range(C):
            current_x = x[n, c, (d * stride):((d * stride)+HH), (r * stride):((r * stride)+WW)]
            # find the maximum
            current_max = np.amax(current_x)
            out[n, c, d, r] = current_max
    ...
    for n in range(N):
      ...
      for r in range(0, steps_to_right):
        ...
        for d in range(0, steps_to_down):
          # iterate through every channel
          for c in range(C):
            current_dout = dout[n, c, d, r]
            current_x = x[n, c, (d * stride):((d * stride)+HH), (r * stride):((r * stride)+WW)]
            # find the index of the maximum element
            current_max_i, current_max_j = np.argmax(current_x) // current_x.shape[1] , (np.argmax(current_x) - (current_x.shape[1] * (np.argmax(current_x) // current_x.shape[1])))
            
            org_i, org_j = ((d * stride) + current_max_i), ((r * stride) + current_max_j) # convert filter coordinates to x coordinates
            
            dx[n ,c , org_i, org_j] += current_dout
