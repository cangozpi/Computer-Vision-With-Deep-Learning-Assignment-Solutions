...
def sgd_momentum(w, dw, config=None):
    ...
    v_t1 = (config["momentum"] * v) - (config["learning_rate"] * dw)
    next_w = w +  v_t1
    
    # v_t+1 becomes v_t in the next iteration so update that in the config
    v = v_t1
...
def sgd_nesterov_momentum(w, dw, config=None):
    ...
    v_t1 = (config["momentum"] * v) - (config["learning_rate"] * dw) # velocity update stays the same
    next_w = w - (config["momentum"] * v) + (1 + config["momentum"]) * v_t1 # position update changes form 
